...
function Todo({
  ...
  return (
    <li className="ml-10 mt-5" style={style}>
      ...
      ${isCompleted ? 'text-gray-600' : 'text-purple-600'}
      `}>
        <CardHeader>
          <CardTitle
            className={isCompleted ? 'text-gray-500' : 'text-purple-600'}>
            {title}
          </CardTitle>
        </CardHeader>
...
export default function Todos({ todos }: { todos: TodoType[] }) {
  return (
    <ul className="mt-16 flex flex-wrap">
      {todos.map((todo) => (
        <Todo
          key={todo._id as unknown as Key}
          title={todo.title}
          _id={todo._id as unknown as string}
        />
      ))}
    </ul>
  );
